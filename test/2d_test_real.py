...
df, properties = get_data(DataParameters("computational_data", ["Density (g/cm3)",
                        "Electric conductivity (S/m)"]))
xs = torch.tensor(df[properties].to_numpy())
ys = torch.tensor(df[["Density (g/cm3)", "Electric conductivity (S/m)"]].to_numpy())
print(ys.min(0))
sys.exit()
...
def generate_next_candidate(x, y, x_test, n_candidates=1):
    ...
    for _ in range(n_candidates):
        ...
        weights = sample_simplex(2).view(-1)
...
def plot_candidates(candidates):
    plt.scatter(ys[:, 0].view(-1), ys[:, 1].view(-1))
    plt.scatter(candidates[:, 0].view(-1), candidates[:, 1].view(-1), c="r")
    #plt.axhline(500, c="r", linewidth=2)
